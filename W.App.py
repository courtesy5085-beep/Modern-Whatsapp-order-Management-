...
# Add Order
    ...
    with col2:
        ...
        st.info(
            ...
            "```\n\n"
            "Features:\n"
            "- Auto-parsing works with messy text\n"
            "- Duplicate detection\n"
...
# Manage Orders
    ...
    if df.empty:
        st.warning("No orders yet. Go to 'Add Order' to create one.")
    else:
        col1, col2, col3 = st.columns(3)
        status_filter = col1.multiselect("Filter Status", df["status"].unique(), default=df["status"].unique())
        date_filter = col2.date_input("From Date", datetime.now() - timedelta(days=30))
        search = col3.text_input("🔍 Search Customer/Product")

        filtered_df = df[
            (df["status"].isin(status_filter)) &
            (df["order_date"] >= pd.Timestamp(date_filter))
        ]
        if search:
            filtered_df = filtered_df[
                filtered_df["customer"].str.contains(search, case=False, na=False) |
        ...
        if st.button("💾 Save Changes", type="primary"):
            update_orders(grid_response['data'])
